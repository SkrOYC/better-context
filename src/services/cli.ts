...
For more detailed help, use: btca <command> --help`
	...
	private async setupCommands(): Promise<void> {
		...
		const availableTechnologies = repos
			.map((repo) => repo.name)
			.sort()
			.join(', ');
		...
		this.program
			...
			.addOption(
				new Option('-t, --tech <technology>', 'technology to ask about')
...
EXAMPLES:
  ...
  $ btca config repos remove --name docs # Remove a repo`
		...
		configCommand
			...
			.addOption(
				new Option('-p, --provider <provider>', 'AI provider').choices([
					'opencode',
					'openai',
					'anthropic',
					'google',
...
EXAMPLES:
  ...
  $ btca config repos clear`
		...
		reposCommand
			...
			.addOption(
				new Option('-u, --url <url>', 'repository URL')
			...
			.addOption(
				new Option('-b, --branch <branch>', 'branch to use')
					.argParser((value: string) => {
						// Basic git branch name validation
						if (!/^[a-zA-Z0-9._/-]+$/.test(value)) {
							throw new Error(
								`Invalid branch name: ${value}. Branch names can only contain letters, numbers, dots, underscores, forward slashes, and hyphens.`
							);
						}
						...
						if (
							...
							throw new Error(
								`Invalid branch name: ${value}. Branch names cannot start/end with dots or slashes, or contain double dots or slashes.`
							);
						}
						return value;
					})
...
EXAMPLES:
  ...
  $ btca config repos add -n vue --url https://github.com/vuejs/core --notes "Vue.js framework"`
			...
			.action(async (options) => {
				await this.handleConfigReposAddCommand(
					options.name,
					options.url,
					options.branch,
					options.notes
...
EXAMPLES:
  ...
  $ btca config repos remove -n typescript`
	...
	private async handleConfigReposAddCommand(
		name: string,
		url: string,
		branch: string,
		notes?: string
	...
	private async handleConfigReposRemoveCommand(name: string): Promise<void> {
		const confirmed = await askConfirmation(
			`Are you sure you want to remove repo "${name}" from config? (y/N): `
		);
		if (!confirmed) {
