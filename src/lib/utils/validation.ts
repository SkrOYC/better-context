...
import { InvalidProviderError, InvalidModelError, OcError } from '../errors';
...
export const validateProviderAndModel = async (
	...
	if (!connected.includes(providerId)) {
		throw new OcError(
			`Provider "${providerId}" is not connected. Connected providers: ${connected.join(', ')}. Run "opencode auth" to configure provider credentials.`
		);
	}
