...
describe('OcService Basic Functionality', () => {
	...
	mockConfigService.getRepos = mock(() => [
		{ name: 'test-tech', path: '/tmp/test', url: 'http://test', branch: 'main' }
	]);
	...
	mockConfigService.rawConfig = mock(() => ({
		reposDirectory: '/tmp/repos',
		repos: [],
		provider: 'test-provider',
		model: 'test-model',
	...
	mockConfigService.cloneOrUpdateOneRepoLocally = mock(
		async () => ({ name: 'test-tech', url: 'http://test', branch: 'main' }) as any
	);
